...
def main():
    ...
    for brick_tsv in brick_files:
        ...
        table_def = {
            ...
            "bronze_path": f"s3a://cdm-lake/tenant-general-warehouse/bervodata/datasets/chess-source/bronze/{brick_name}.csv"
        }
    ...
    tables.append({
        ...
        "bronze_path": "s3a://cdm-lake/tenant-general-warehouse/bervodata/datasets/chess-source/bronze/ddt_ndarray.csv"
    })
    ...
    tables.append({
        ...
        "bronze_path": "s3a://cdm-lake/tenant-general-warehouse/bervodata/datasets/chess-source/bronze/sys_ddt_typedef.csv"
    })
    ...
    table_comments["sys_ddt_typedef"] = [
        ...
        {"name": "original_csv_string", "type": "STRING", "comment": '{"description": "Original representation of this variable in the CHESS data dump CSV"}'},
    ]
    ...
    bervodata_json = {
        ...
        "is_tenant": True,
        "paths": {
            "data_plane": "s3a://cdm-lake/tenant-general-warehouse/bervodata/",
            "bronze_base": "s3a://cdm-lake/tenant-general-warehouse/bervodata/datasets/chess-source/bronze",
            "silver_base": "s3a://cdm-lake/tenant-sql-warehouse/bervodata/chess.db"
        },
        "defaults": {
            "csv": {
                "header": True,
    ...
    # Write JSON file
    json_path = output_dir / "chess.json"
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(bervodata_json, f, indent=2)
    ...
    print(f"Created {len(tables)} table definitions in chess.json")
